...
def main():
    ...
    for (_, left), (_, centre), (_, right) in zip(
            calls.iterrows(), 
    ...
    # ---------------------------------
    plt.figure(figsize=(9,6))
    pdf_norm = scipy.interpolate.interp1d(
        bflys.strike, normalised_prob, kind="cubic", fill_value="extrapolate"
    )
    x_new = np.linspace(bflys.strike.min(), bflys.strike.max(), 1000)
